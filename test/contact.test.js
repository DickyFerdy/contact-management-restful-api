  ...
  it("should reject if request is not valid", async () => {
    const result = await supertest(web)
      ...
      .send({
        ...
        last_name: 'test',
        email: 'test@mail.com',
        phone: '081234567890000000000000'
      });
...
describe("PUT /api/contacts/:contactId", function () {
  ...
  it("should can update existing contact", async () => {
    ...
    const result = await supertest(web)
      ...
      expect(result.status).toBe(200);
      expect(result.body.data.first_name).toBe("Dicky");
      expect(result.body.data.last_name).toBe("Ferdy");
      expect(result.body.data.email).toBe("kyy@mail.com");
      expect(result.body.data.phone).toBe("081230000");
  });
